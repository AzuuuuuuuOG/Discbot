...
betalte_personer = load_data()  # Indlæs data ved opstart
...
# Rolle-IDs
rolle_id_1 = 1209253241653698650  # Erstat med den faktiske rolle-ID
...
JSON_FILE_PATH = 'unikke_ids.json'
...
async def on_reaction_add(reaction, user):
    if str(reaction.emoji) == '✅' and reaction.message.channel.id in [salg_kanal_id, lab_kanal_id]:
        ...
        gem_data(unikke_ids)  # Gem opdateret opgave data til JSON-filen
...
async def ADMIN_NULSTIL(ctx):
    ...
    gem_data(unikke_ids)  # Gem den nulstillede data til JSON-filen
...
async def søgopgaver(ctx, user_id: int):
    ...
    if tasks_for_user:
        ...
        await asyncio.sleep(5)  # Vent i 5 sekunder
        await ctx.message.delete()  # Slet brugerens kommandobesked efter 5 sekunder
        await response_message.delete()  # Slet botten svar efter yderligere 5 sekunder
...
def fjern_alle_brugere():
    gem_data({})  # Overskriver JSON-filen med en tom dictionary
...
bot.run('MTIwOTIyMDQ4ODA2OTY0NDMzOA.GtBHoX.3jjOjK9MukEoD1RZto0nhFcko_R_y1tmB17JnU')  # Erstat 'TOKEN' med din bot's token
